{-
 -  CallByReferenceSpec.hs
 -
 -  Reference implementation of the toy language CALL_BY_REFERENCE from the
 -  EOPL3 textbook by Mitchell Wand.
 -
 -  This module provides a test suite for the CALL_BY_REFERENCE interpreter.
 -
 -  Author: Matthew A Johnson
 -}
module CallByReferenceSpec (spec) where

import Data.Either (fromRight)
import HOPL.CALL_BY_REFERENCE.DataStructures (ExpVal (..))
import HOPL.CALL_BY_REFERENCE.Environment (Env (emptyEnv, extendEnv'))
import HOPL.CALL_BY_REFERENCE.Interp (interpWith)
import HOPL.CALL_BY_REFERENCE.Store (emptyStore)
    ...
    describe "Value tests" $ do
      ...
      specify "cbr-direct-aliasing-1" $
        interp
          ...
          \   in ((p b) b)"
        `shouldBe` NumVal 44
      specify "cbr-indirect-aliasing-1" $
        ...
        interp
          ...
          \      in ((p b) (q b))"
          `shouldBe` NumVal 33
      specify "cbr-indirect-aliasing-2" $
        -- in this language, you can't return a reference.
        interp
          "let p = proc (x) proc (y) \
          \           begin \
