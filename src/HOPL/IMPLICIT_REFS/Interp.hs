{-
 ...
 -}
module HOPL.IMPLICIT_REFS.Interp
  ( interp,
    interpWith,
    interpWith',
  )
...
import HOPL.IMPLICIT_REFS.DataStructures (DenVal, Environment, ExpVal (..), Procedure (..))
import HOPL.IMPLICIT_REFS.Environment (Env (..))
import HOPL.IMPLICIT_REFS.Lang.Parser (ParseError, parseToplevel)
